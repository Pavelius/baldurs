...
struct aref {
	...
	constexpr aref(T* v, unsigned count) : data(v), count(count) {}
	template<unsigned N> constexpr aref(adat<T, N>& v) : data(v.begin()), count(v.getcount()) {}
	template<unsigned N> constexpr aref(T(&v)[N]) : data(v), count(N) {}
	constexpr const T& operator[](unsigned index) const { return data[index]; }
	constexpr T& operator[](unsigned index) { return data[index]; }
	explicit operator bool() const { return count != 0; }
