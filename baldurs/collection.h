...
struct adat {
	...
	bool					consist(const void* e) { return e >= data && e <= data + count; }
	void					clear() { count = 0; }
...
template<typename T, unsigned maximum, typename E> class aset {
	struct pair {
		E					id;
		T					value;
	};
...
public:
	constexpr aset() : data() {};
	constexpr aset(const std::initializer_list<pair>& list) {
		for(auto& e : list)
			data[e.id] = e.value;
	}
