...
bool draw::dlgask(const char* text) {
	...
	while(ismodal()) {
		...
		button(278, 289, buttonok, 0, 0, res::GBTNSTD, "OK", KeyEnter);
		button(398, 289, buttoncancel, 1, 0, res::GBTNSTD, "Отмена", KeyEscape);
		domodal();
	}
	return getresult();
}
...
int draw::dlgcho(const char* text, const char* a1, const char* a2) {
	...
	while(ismodal()) {
		...
		button(285, 196, choose_answer, 1, 0, res::GBTNLRG2, a1, Alpha + '1');
		button(285, 228, choose_answer, 2, 0, res::GBTNLRG2, a2, Alpha + '2');
		button(344, 260, buttoncancel, 0, 0, res::GBTNSTD, "Отмена", KeyEscape);
		domodal();
	}
	return getresult();
}