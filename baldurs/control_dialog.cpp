...
void draw::dlgmsg(const char* text) {
	...
	while(ismodal()) {
		...
		button(346, 276, buttonok, 0, res::GBTNSTD, "Продолжить", KeyEnter);
		domodal();
	}
}
...
bool draw::dlgask(const char* text) {
	...
	while(ismodal()) {
		...
		button(398, 289, buttoncancel, 0, res::GBTNSTD, "Отмена", KeyEscape);
		domodal();
	}
	return getresult();
}
...
int draw::dlgcho(const char* text, const char* a1, const char* a2) {
	...
	while(ismodal()) {
		...
		button(344, 260, buttoncancel, 0, res::GBTNSTD, "Отмена", KeyEscape);
		domodal();
	}
	return getresult();
}