...
{"Damage", "Урон", {Bludgeon, Sound}, BSVRN(damagei)},
...
{"Monster", "Монстр", {}, BSVRN(monsteri)},
{"Name", "Имя"},
{"Position", "Позиция"},
{"Race", "Раса", {FirstRace, LastRace}, BSVRN(racei)},
{"Reaction", "Реакция", {}, &bsdata<reactioni>::source},
...
const char* getstr(const variant& id) {
	switch(id.type) {
	case Creature: return id.getcreature()->getname();
	case Door: return "Door";
	case ItemGround: return id.getitemground()->getname();
	case Container: return id.getcontainer()->name;
	}
