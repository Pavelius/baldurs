...
void stringbuilder::sep(const char* first_time, const char* start, const char* separator) {
	if(start == p) {
		if(first_time)
			addv(first_time, 0);
	} else {
...
void stringbuilder::addv(const char* format, const char* format_param, const char* prefix, const char* suffix) {
	if(prefix)
		addv(prefix, 0);
